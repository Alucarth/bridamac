...
<div class="row">
	...
	<div class="row">
		<div class="col-md-10">
			...
			{{ Former::select('payment_type_id')->addOption('','')->label('Tipo de Pago')
				->fromQuery($paymentTypes, 'name', 'id') }}			
			{{ Former::text('payment_date')->label('Fecha de Pago')->append('<i class="glyphicon glyphicon-calendar"></i>') }}
			{{ Former::text('transaction_reference')->label('Referencia de la Transacción') }}
	...
	<center class="buttons">
	 	<a href="{{ url('pagos/') }}" class="btn btn-info" role="button">Cancelar</a>
	...
	function populateInvoiceComboboxes(clientId, invoiceId) {
	  ...
	  if (invoiceId) {
	    ...
	    setComboboxValue($('.invoice-select'), invoice.public_id, (invoice.invoice_number + ' - ' +
	            invoice.invoice_status.name + ' - ' + client.name + ' - ' +
