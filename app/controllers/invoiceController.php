...
//            $invoice->logo = $type_document->logo;
	...
	public function sendInvoiceByMail()
	{
        	...
        	//Session::flash('message',"Enviado corr茅ctamente");
        	}
        	//else
        //		Session::flash('error',"No ingres贸 el mail del remitente");
            return View::make('factura.index', array('invoices' => $invoices));
	}
	...
	private function sendByMail(){
				...
				if($aux == 0)
				{
					$errorMessage = trans('El cliente no tiene Correo Electr脙鲁nico.');
	...
	private function save($publicId = null)
	{
		...
		$errorS = "Expir脙鲁 la fecha l脙颅mite de " . $branch->name;
		if($a - $b < 0)
		{
...
//        return 0;
       ...
       foreach ($results as $key => $res){
        ...
        Session::flash('message','Se importaron '.$cont.' facturas exit贸samente');
        $invoices = Invoice::where('account_id',Auth::user()->account_id)->where('branch_id',Session::get('branch_id'))->orderBy('public_id', 'DESC')->get();
        return View::make('factura.index', array('invoices' => $invoices));
        return 0;
    }
